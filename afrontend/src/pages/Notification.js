...
const Notification = () => {
  ...
  const handleAllRead = async () => {
    try {
      ...
      const res = await axios.post(
        "/api/auth/user/getNotifications",
        { userId: user._id },
        {
  ...
  const handleDeleteAllRead =async (req,res) => {
    try {
      dispatch(showLoading())
      const response=await axios.post('/api/auth/user/deleteNotifications',{userId:user._id},{headers:{Authorization:`Bearer ${localStorage.getItem("token")}`}})
      dispatch(hideLoading());
      ...
      console.log(error);
      message.error("omething went wring in deleteing")    }
  };
  return (
    <Layout>
      <h4 className="p-3 text-center" style={{ color: "white" }}>
        Notification
      </h4>
      <Tabs>
        <Tabs.TabPane
          tab={<span style={{ color: "white" }}>UnRead</span>}
          key={0}
          style={{ backgroundColor: "gray", color: "white" }}
        >
          <div className="d-flex justify-content-end">
            <h4 className="p-2 " onClick={handleAllRead}style={{cursor:"pointer"}}>
              Mark all read
            </h4>
          </div>
        ...
        <Tabs.TabPane
          tab={<span style={{ color: "white" }}>Read</span>}
          key={1}
          style={{ backgroundColor: "gray", color: "white" }}
        >
          <div className="d-flex justify-content-end">
